const google = require('../assets/google.png');
const apple = require('../assets/apple.png');
const logo = require('../assets/logo.png');
const logo2 = require('../assets/logo2.png');

const back = require('../assets/left_arrow.png');
const next = require('../assets/next.png');

const setting = require('../assets/setting.png');
const add = require('../assets/add_user.png');
const clock = require('../assets/time.png');
const arrow_up = require('../assets/arrow_up.png');
const arrow_down = require('../assets/arrow_down.png');
const emoji = require('../assets/emoji.png');
const close = require('../assets/close.png');
const search = require('../assets/search.png');
const accept = require('../assets/accept.png');

const user = require('../assets/user.png');
const edit = require('../assets/edit.png');
const email = require('../assets/email.png');

const notification = require('../assets/notification.png');
const bin = require('../assets/bin.png');
const invite = require('../assets/invite.png');
const star = require('../assets/star.png');
const feedback = require('../assets/feedback.png');
const exit = require('../assets/exit.png');
const rightArrow = require('../assets/right-arrow.png');
const call = require('../assets/call.png');
const privacy = require('../assets/file.png');

export default {
  google,
  apple,
  logo,
  logo2,
  back,
  next,
  setting,
  add,
  clock,
  arrow_down,
  arrow_up,
  emoji,
  close,
  search,
  accept,
  user,
  edit,
  email,
  notification,
  bin,
  invite,
  star,
  feedback,
  exit,
  rightArrow,
  call,
  privacy,
};
